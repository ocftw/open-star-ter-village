...
import { ActionExecutionOptions, ActionValidationError, validateContributeJoinedProjects } from '@/game/core/stage/action/validate';
import { applyActionCost } from './applyActionCost';

export type ContributeJoinedProjects = (
  contributions: ContributionAction[],
  options?: ActionExecutionOptions,
) => void;

export const contributeJoinedProjects: GameMove<ContributeJoinedProjects> = ({ G, playerID }, contributions, options) => {
  // Shared validation: same predicates the client preflight runs.
  const result = validateContributeJoinedProjects(G, playerID, contributions, options);
  if (!result.valid) {
    throw new ActionValidationError(result);
  }

  // All checks passed — now mutate state
  applyActionCost(G, playerID, 'contributeJoinedProjects', options);
