 ...
 * validators and this resolution share the same occupied-slot rule, so a
 * `useOvertime` request against a free slot simply executes normally without
 * consuming the token.
 */
export const applyActionCost = (
  ...
  if (options?.useOvertime && ActionSlotSelector.isOccupied(slot)) {
